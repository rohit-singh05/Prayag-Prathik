...
export default function HomePage() {
  ...
  const [texts, setTexts] = useState({
    ...
    description: "Explore the holy city with our optimized Tourist Transport Planner.",
  ...
  useEffect(() => {
    const doTranslation = async () => {
      if (language === "en") {
        ...
        const [heading, description, button, ariaPrev, ariaNext, footer] = await Promise.all([
          translateText("Welcome to Prayagraj", language),
          translateText("Explore the holy city with our optimized Tourist Transport Planner.", language),
          translateText("Start Exploring", language),
          translateText("Previous image", language),
          translateText("Next image", language),
          translateText("© 2025 Prayagraj Tourist Transport Planner", language),
        ]);
        setTexts({ heading, description, button, ariaPrev, ariaNext, footer });
  ...
  return (
    <div className="relative min-h-screen flex flex-col bg-gray-50 text-gray-800">
      ...
      <div className="sticky top-0 z-[900] flex items-center justify-between px-6 py-4 bg-white/90 backdrop-blur-md shadow-md border-b border-emerald-100">
        <Typography
          variant="h6"
          className="text-emerald-700 font-semibold tracking-wide"
        >
          Plan Your Visit
        </Typography>
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="border border-emerald-300 rounded-md px-3 py-1.5 text-sm bg-white text-gray-700 font-medium shadow-sm hover:border-emerald-400 focus:outline-none focus:ring-2 focus:ring-emerald-400 transition-all duration-200"
          >
      ...
      <div className="relative flex-1 flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-20 text-center px-4 max-w-md bg-white/95 backdrop-blur-sm rounded-xl shadow-md p-6 space-y-4 border border-emerald-200">
          <h1 className="text-2xl font-semibold text-emerald-700">{texts.heading}</h1>
          ...
          <div className="flex justify-center mt-4 space-x-2">
            {images.map((_, index) => (
              <button
                ...
                className={`w-3 h-3 rounded-full transition-all duration-300 ${index === currentIndex ? "bg-emerald-600 scale-110" : "bg-gray-400/50"
                  }`}
